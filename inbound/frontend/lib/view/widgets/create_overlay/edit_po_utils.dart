...
Future<void> editPo(id, body, context) async {
  final model = Provider.of<PurchaseOrderChangeNotifier>(context, listen: false);
  await model.editPurchaseOrder(id, body, const {
    'Content-Type': 'application/json',
  });

  SnackBar snackBar = CustomSnackBarLoaded(context, "Successfully edited purchase order");


  if (model.mainPageRequestSate == PageState.error) {
    snackBar = CustomSnackBarError(context, "Failed to edit purchase order: ${model.errorMessage}");
  }
